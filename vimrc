...
iabbrev ## Name:lala     "绑定输入##替换为名字
augroup Echom
    autocmd!
    autocmd BufWrite * :echom "QAQ"
augroup END              "设置messages多输出一个 QAQ
...
set statusline=[文件路径=%F%m%r%h%w]\ [文件系统=%{&ff}]\ [文件类型=%Y]\ [位置=%l,%v]\ [%%%p]\ [时间=%{strftime(\"%y/%m/%d\ -\ %H:%M\")}]
